...
from core_fewshot.LMs.model import BertClassifier, BertClaInfModel
from core_fewshot.data_utils.dataset import Dataset
from core_fewshot.data_utils.load import load_data
from core_fewshot.utils import init_path, time_logger
...
class LMTrainer():
    def __init__(self, cfg):
        ...
        self.output_dir = f'/dev/shm/ssy/output/{self.dataset_name}{self.use_gpt_str}/{self.model_name}-seed{self.seed}'
        self.ckpt_dir = f'prt_lm_fewshot/{self.dataset_name}{self.use_gpt_str}/{self.model_name}-seed{self.seed}'
        ...
        self.n_labels = max(data.y) + 1 # data.y.unique().size(0)
        
        tokenizer = AutoTokenizer.from_pretrained(self.model_name, cache_dir='/dev/shm/ssy')
        ...
        bert_model = AutoModel.from_pretrained(self.model_name, cache_dir='/dev/shm/ssy')
    ...
    def eval_and_save(self):
        if self.model_name == 'microsoft/deberta-base':
            print('deberta-base')
            feat_size = 768
        else:
            print('e5-large')
        ...
        emb = np.memmap(init_path(f"{self.ckpt_dir}.emb"),
                        dtype=np.float16,
                        mode='w+',
                        shape=(self.num_nodes, self.feat_shrink if self.feat_shrink else feat_size))
